...
class CyberSource
{
	...
	public $card = [];
	public $items = [];
	public $currency = "USD"; // If no currency is passed during the charge call, then consider default as USD
	/**
	...
	public $result_codes = [
		...
		'110' => 'Only a partial amount was approved. Try again or use different payment method',
		'150' => 'Error: General system failure. Try again or use different payment method',
		'151' => 'Error: The request was received but there was a server timeout. Try again or use different payment method',
		'152' => 'Error: The request was received, but a service did not finish running in time. Try again or use different payment method',
		'200' => 'The authorization request was approved by the issuing bank but declined by CyberSource because it did not pass the Address Verification Service (AVS) check. Try again or use different payment method',
		'201' => 'The issuing bank has questions about the request. Try again or use different payment method',
		'202' => 'Expired card. Try again or use different payment method',
		'203' => 'General decline of the card. Try again or use different payment method',
		'204' => 'Insufficient funds in the account. Try again or use different payment method',
		'205' => 'Stolen or lost card. Try again or use different payment method',
		'207' => 'Issuing bank unavailable. Try again or use different payment method',
		'208' => 'Inactive card or card not authorized for card-not-present transactions. Try again or use different payment method',
		'209' => 'American Express Card Identification Digits (CID) did not match. Try again or use different payment method',
		'210' => 'The card has reached the credit limit. Try again or use different payment method',
		'211' => 'Invalid CVN. Try again or use different payment method',
		'221' => 'The customer matched an entry on the processor\'s negative file. Try again or use different payment method',
        '222' => 'Decline - customer\'s account is frozen. Try again or use different payment method',
		'230' => 'The authorization request was approved by the issuing bank but declined by CyberSource because it did not pass the CVN check. Try again or use different payment method',
		'231' => 'Invalid credit card number. Try again or use different payment method',
		'232' => 'The card type is not accepted by the payment processor. Try again or use different payment method',
		'233' => 'General decline by the processor. Try again or use different payment method',
		'234' => 'There is a problem with your CyberSource merchant configuration. Try again or use different payment method',
		'235' => 'The requested amount exceeds the originally authorized amount. Try again or use different payment method',
		'236' => 'Processor failure. Try again or use different payment method',
		'237' => 'The authorization has already been reversed. Try again or use different payment method',
		'238' => 'The authorization has already been captured. Try again or use different payment method',
		'239' => 'The requested transaction amount must match the previous transaction amount. Try again or use different payment method',
		'240' => 'The card type sent is invalid or does not correlate with the credit card number. Try again or use different payment method',
		'241' => 'The request ID is invalid. Try again or use different payment method',
		'242' => 'You requested a capture, but there is no corresponding, unused authorization record. Try again or use different payment method',
		'243' => 'The transaction has already been settled or reversed. Try again or use different payment method',
		'246' => 'The capture or credit is not voidable because the capture or credit information has laready been submitted to your processor. Or, you requested a void for a type of transaction that cannot be voided. Try again or use different payment method',
		'247' => 'You requested a credit for a capture that was previously voided. Try again or use different payment method',
		'250' => 'Error: The request was received, but there was a timeout at the payment processor. Try again or use different payment method',
		'520' => 'The authorization request was approved by the issuing bank but declined by CyberSource based on your Smart Authorization settings. Try again or use different payment method',
        '481' => 'The order has been rejected by Decision Manager.Try again or use different payment method',
	];
	public $card_types = [
		'Visa'             => '001',
		'MasterCard'       => '002',
	...
	public function add_item($price, $quantity = 1, $additional_fields = [])
	{
		$item = [
			'unitPrice' => $price,
			'quantity'  => $quantity,
		...
		if ($response->decision != 'ACCEPT') {
			...
			if ($response->reasonCode == 101) {
				if (!isset($response->missingField)) {
					...
					if (is_array($response->missingField)) {
	...
	public function card_type($card_number)
	{
		...
		if (strlen($card_number) == 15 && $digits[0] == 3 && ($digits[1] == 4 || $digits[1] == 7)) {
			...
			if (strlen($card_number) == 14 && $digits[0] == 3 && in_array($digits[1], [0, 6, 8])) {
				...
				if (strlen($card_number) == 16 && (
					...
					if (strlen($card_number) == 15 && in_array(substr($card_number, 0, 4), [2014, 2149])) {
						return 'enRoute';
					} else {
						if (strlen($card_number) == 16 && (
								in_array(substr($card_number, 0, 4), [3088, 3096, 3112, 3158, 3337]) ||
								(substr($card_number, 0, 8) >= 35280000 && substr($card_number, 0, 8) <= 35899999)
							)
						) {
							return 'JCB';
						} else {
							if (strlen($card_number) == 16 && $digits[0] == 5 && $digits[1] >= 1 && $digits[1] <= 5) {
								return 'MasterCard';
							} else {
								if ((strlen($card_number) == 13 || strlen($card_number) == 16) && $digits[0] == 4) {
									return 'Visa';
								}
							}
						}
